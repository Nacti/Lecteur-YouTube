...
class ChansonCell: UITableViewCell {
    ...
    func telechargerImage() {
        ...
        miniature.image =  nil
        // Vérifie que la constante créée est une URL qui renvoi un string correspondant au champs miniatureUrl créé dans Chanson.swift
        if let url = URL(string: self.CellChanson.miniatureUrl) {
            ...
            // Si pas de réponse, on arrête la requête
            task.resume()
        }
    }
    ...
    func creerCell(_ parametreChanson: ClassChanson) {
        // Récupération chanson tenant correspondant à la variable créée au début de la classe ChansonCell
        self.CellChanson = parametreChanson
        // Ajout de la fonction telechargerImage
        telechargerImage()
